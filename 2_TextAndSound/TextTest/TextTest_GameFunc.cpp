...
SDL_FRect g_title_en_rect;

SDL_Texture* g_title_kr;
SDL_FRect g_title_kr_rect;
...
void InitGame() {
	...
	// English Title
	{
		...
		SDL_Surface* tmp_surface = TTF_RenderText_Blended(font1, str.c_str(), str.size(), red);

		g_title_en_rect.x = 0;
		g_title_en_rect.y = 0;
		g_title_en_rect.w = (float)tmp_surface->w;
		g_title_en_rect.h = (float)tmp_surface->h;
	...
	// Korean Title
	{
		...
		g_title_kr_rect.x = 0;
		g_title_kr_rect.y = 0;
		g_title_kr_rect.w = (float)tmp_surface->w;
		g_title_kr_rect.h = (float)tmp_surface->h;
...
void Render() {
	...
	// Draw Title (English)
	{
		...
		SDL_RenderTexture(g_renderer, g_title_en, &g_title_en_rect, &tmp_r);
	}
	...
	// Draw Title (Korean)
	{
		...
		SDL_RenderTexture(g_renderer, g_title_kr, &g_title_kr_rect, &tmp_r);
	}
